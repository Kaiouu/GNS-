...
function price(string calldata name) public pure returns (uint) {
  ...
  string memory json = Base64.encode(
    abi.encodePacked(
        '{"name": "',
        _name,
        '", "description": "A domain on the Guilder name service", "image": "data:image/svg+xml;base64,',
...
function grtRecord(string calldata name) public view returns(string memory) {
  return records[name];
}
}